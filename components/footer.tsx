...
export function Footer() {
  return (
    <footer className="relative overflow-hidden bg-ink text-white">
      ...
      <div className="relative mx-auto grid max-w-7xl gap-10 px-5 py-16 sm:px-6 md:grid-cols-2 lg:grid-cols-[1.35fr_0.65fr_0.8fr_0.9fr] lg:px-8">
        <div>
          <Link href="/" className="flex items-center gap-4">
            ...
            <span>
              <span className="block whitespace-nowrap text-lg font-extrabold tracking-[0.16em] text-white">MK DESINGZ</span>
          ...
          <div className="mt-7 flex gap-3">
            <Link href="https://www.instagram.com/_mkdesignz_/" target="_blank" className="flex h-11 w-11 items-center justify-center rounded-full border border-white/10 bg-white/5 text-white transition hover:-translate-y-1 hover:bg-white hover:text-ink">
              <Camera size={18} />
            </Link>
            <Link href={buildWhatsappUrl(defaultWhatsappMessage)} target="_blank" className="flex h-11 w-11 items-center justify-center rounded-full border border-white/10 bg-white/5 text-white transition hover:-translate-y-1 hover:bg-white hover:text-ink">
              <MessageCircle size={18} />
            </Link>
            <Link href="mailto:mkdesingz@example.com" className="flex h-11 w-11 items-center justify-center rounded-full border border-white/10 bg-white/5 text-white transition hover:-translate-y-1 hover:bg-white hover:text-ink">
              <Mail size={18} />
            </Link>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="mt-5 grid gap-3">
            {navItems.map((item) => (
              <Link key={item.href} href={item.href} className="text-sm font-semibold text-white/58 transition hover:text-white">
                {item.label}
              </Link>
            ))}
          </div>
        ...
        <div>
          ...
          <div className="mt-5 grid gap-3">
            {serviceLinks.map((service) => (
              <Link key={service} href="/services" className="text-sm font-semibold text-white/58 transition hover:text-white">
                {service}
              </Link>
            ))}
          </div>
        ...
        <div>
          ...
          <div className="mt-5 grid gap-4 text-sm font-semibold text-white/62">
            <Link href={buildWhatsappUrl(defaultWhatsappMessage)} target="_blank" className="flex items-center gap-3 hover:text-white">
              <Phone size={16} /> 7305514199
            </Link>
            <Link href="mailto:mkdesingz@example.com" className="flex items-center gap-3 hover:text-white">
